...
function corruptionChecksum2(spreadsheet) {
  const rows = spreadsheet.split("\n");
  let result = 0;

  for(let i = 0; i < rows.length; i++) {
    const items = rows[i].split(/\t|\s+/).map(Number);
    const [number, divisor] = getDivisible(items);
...
function getDivisible(numbers) {
  for (let number of numbers) {
    const divisor = numbers.find((_number) => _number !== number && number % _number === 0);
    if (divisor) {
...
console.log(corruptionChecksum2(
`5 9 2 8
9 4 7 3
3 8 6 5`)); // 9
